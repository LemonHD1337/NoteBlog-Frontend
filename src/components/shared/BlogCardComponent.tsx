...
function BlogCardComponent(props: BlogComponentProps) {
    return(
        <article className={"" + props.className}>
            <BlogCardContext.Provider value={props.blog}>
                {props.children}
...
BlogCardComponent.Title = function TitleComponent(props: DefaultProps) {
    ...
    return <h1 className={"mb-6 " + props.className}>{blog.title} {props.children}</h1>
}
...
BlogCardComponent.Img = function ImgComponent(props: DefaultProps ) {
    ...
    return <img src={import.meta.env.VITE_URL_UPLOADS + blog.image} alt={"blog main picture"} className={" h-[230px] object-contain mb-6 " + props.className}/>
}
