...
const PaginationComponent = (props: PaginationComponentProps) => {
	const renderItem = (item: PaginationRenderItemParams) => {
		if (item.type === "next") {
			return <PaginationItem {...item} onClick={() => props.setPage(prevState => prevState + 1)} />;
		} else if (item.type === "previous") {
			return <PaginationItem {...item} onClick={() => props.setPage(prevState => prevState - 1)}></PaginationItem>;
		} else {
			return (
				<PaginationItem
					{...item}
	...
	return (
		<Pagination
			...
			siblingCount={2}
			count={props.count}
			className={"Pagination1 " + props.className}
